...
def create_gui(toggle_trigger, update_trigger_key, toggle_esp, toggle_defuse, update_esp_config):
    dpg.create_context()
    ...
    with dpg.window(label="Cs2 Cheat", width=window_width, height=window_height, no_move=True, no_resize=True, no_title_bar=False, no_scrollbar=True, no_collapse=True):
        with dpg.tab_bar():
            ...
            with dpg.tab(label="Aim"):
                ...
                dpg.add_input_text(label="Trigger Key", default_value="x", callback=update_trigger_key)
